---
title: "Simulation Analyses for EMP Population"
...
```{r}
allSims <- read.table(file = "/Users/hia5y7/Desktop/R/Simulations/EMP/EMPandAdditive4_Add_QTN_4_Epi_QTN_h.2_0.2_add.eff_0.6_epis.eff_0.6_reps_100/Simulated.Data.100.Reps.Herit.0.2.txt",header=T,stringsAsFactors=F)
```

##Read genotypes
```{r}
genos <- read.csv("/Users/hia5y7/Desktop/R/Simulations/EMP/Idealized_EMP_genos.csv", header = T, stringsAsFactors=F, na.strings=c("-","NA"))[1:192,]
genos <- genos[which(genos[2,] != ".")]
genos[3:nrow(genos),2:ncol(genos)] <- as.numeric(unlist(genos[3:nrow(genos),2:ncol(genos)]))
```

##Create Matrices for data to be inserted in to
```{r}
setwd("/Users/hia5y7/Desktop/R/Simulations/EMP/EMPandAdditive4_Add_QTN_4_Epi_QTN_h.2_0.2_add.eff_0.6_epis.eff_0.6_reps_100/")
sigs <- read.table(file = "Genotypic.information.for.4.Additive.QTN.txt",stringsAsFactors=F,header=T,sep = "\t")[,1]
twosigs <- read.table(file = "Genotypic.information.for.4.Epistatic.QTN.txt",stringsAsFactors=F,header=T, sep = "\t")[,1]
...
for(i in 2:ncol(allSims)){
  ...
  sim <- read.cross(format = "csv", ".", file=paste(i-1,".genos.tmp.csv"), na.strings=c("-","NA"), genotypes = c(0,0.5,1))
  ...
  rownames(lod) <- colnames(genos)[2:ncol(genos)]
...
for(i in 1:100){
  ...
  sim <- read.cross(format = "csv", ".", file=paste(i,".genos.tmp.csv"),
                    na.strings=c("-","NA"), genotypes = c(0,0.5,1))
